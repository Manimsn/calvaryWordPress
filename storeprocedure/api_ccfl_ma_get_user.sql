...
alter PROCEDURE [dbo].[api_ccfl_ma_get_user]    
    @UserID AS INT    
...
BEGIN    
    IF @UserID > 0    
        SELECT u.[User_ID]    
             , u.[User_Name]    
             , COALESCE(NULLIF(LTRIM(RTRIM(u.[Display_Name])), ''), NULLIF(LTRIM(RTRIM(CONCAT(COALESCE(c.First_Name, ''), ' ', COALESCE(c.Last_Name, '')))), ''), NULLIF(LTRIM(RTRIM(c.Nickname)), ''), u.[User_Name], '') AS Display_Name    
             , u.[Password]    
             , u.[Admin]    
             , u.[Domain_ID]    
